...
class Net(nn.Module):
    def __init__(self, s_dim, a_dim):
        ...
        self.a_dim = a_dim
        self.s_dim = s_dim
        self.conv1 = nn.Conv2d(3, s_dim, (1,1), padding=0)
        self.conv2 = nn.Conv2d(3, s_dim, (1,1), padding=0)
        self.conv1_bn = nn.BatchNorm2d(s_dim)
        self.conv2_bn = nn.BatchNorm2d(s_dim)
        self.pi1 = nn.Linear(s_dim*6*6, 100)
        self.pi2 = nn.Linear(100, a_dim)
        self.pi1_bn = nn.BatchNorm1d(100)
        self.v1_bn = nn.BatchNorm1d(100)
        self.v1 = nn.Linear(s_dim*6*6, 100)
    ...
    def forward(self, x):
        c1 = F.relu(self.conv1_bn(self.conv1(x)))
        c2 = F.relu(self.conv2_bn(self.conv2(x)))
    ...
    def choose_action(self, s):
        ...
        if np.random.randint(100) < 50 and self.explore :
